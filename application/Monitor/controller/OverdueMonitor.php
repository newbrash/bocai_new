    ...
    public function start()
    {
        ...
        $redis->psubscribe(array('__keyevent@0__:expired'), function ($redis, $pattern, $chan, $msg) {
            ...
            if (isset($data['tid'])) {
                ...
                ->alias('r')
                ->join('bc_user_user u', 'u.id=r.get_id', 'left')
                ->field('r.*,u.nickname,u.connectionID')
                ...
                if (!$redInfo['status'] && $redInfo['recive_num'] > $redInfo['recived_num']) {
                    ...
                    try {
                        ...
                        if ($redInfo['type']==1) {
                            ...
                            $noticeContent = '(@title:红包过期退还通知@)由于用户“'.$redInfo['nickname'].'”过期未领取红包，红包金额'.$redInfo['money'].'个金币于'.date('Y-m-d H:i:s').'已退还至您的账户当中！';
                        } else {
                            $content = DB::name('chat_room')->field('id,content')->where('id', $data['mid'])->find();
                            $content['content'] = 'true:'.$content['content'];
                            DB::name('chat_room')->update($content);
                            $noticeContent = '(@title:红包过期退还通知@)您于'.date('Y-m-d H:i:s', $redInfo['send_time']).'发的群发红包剩余'.$redInfo['money'].'个金币已退还至您的账户中！';
                        }
                        // 修改红包的领取状态为过期状态，statu=1为领取状态，status=0为待领取状态，status=2为过期状态
                        DB::name('user_red')->where('id', $redInfo['id'])->update(['status'=>2]);
                        $systemNotice = [
                            'send_id'=>-10086,
                            'get_id'=>$redInfo['send_id'],
                            'time'=>time(),
                            'status'=>-1,
