...
class Client extends Controller
{
    ...
    public function addData($play_type = null)
    {
        if (!isset($play_type)) {
            $play_type = '两面';
        }
        $shengXiaoList = ['猪','鼠','牛','虎','兔','龙','蛇','马','羊','猴','鸡','狗'];
        $list = DB::name('lhc_odds')->where('play_type', $play_type)->select();
        foreach ($list as $key => $vo) {
            if (in_array($vo['option_name'], $shengXiaoList)) {
                $vo['ballNum'] = $this->getShengXiao($vo['option_name']);
            }
            if ($vo['type']) {
                $oddsList[$vo['play_type']][$vo['type']][] = $vo;
            } else {
                $oddsList[$vo['play_type']][] = $vo;
            }
        }
        dump($oddsList);
    }
    ...
    public function getShengXiao($index)
    {
        $list = ['猪','鼠','牛','虎','兔','龙','蛇','马','羊','猴','鸡','狗'];
        $year = (intval(Date('Y'))+9)%12;
        $i = array_search($index, $list);
        if ($i == $year) {
            $result = ['01','13','25','37','49'];
        } elseif ($i>$year) {
            $j = $i-$year+1;
            for (;$j<49; $j+=12) {
                if ($j<10) {
        ...
        $data =[
            'wuxin'=> [
                'zhixuan'=>[
                    'fushi'=> $list[0]['odds'],
                    'danshi'=> $list[1]['odds'],
                    'zuhe'=> $list[2]['odds']."~".$list[3]['odds'],
                ],
                'wuxinzuxuan'=>[
                    'zuxuan120'=> $list[4]['odds'],
                    'zuxuan60'=> $list[5]['odds'],
                    'zuxuan30'=> $list[6]['odds'],
                    'zuxuan20'=> $list[7]['odds'],
                    'zuxuan10'=> $list[8]['odds'],
                    'zuxuan5'=> $list[9]['odds']
                ],
            ],
            'daxiaodanshuang'=>[
