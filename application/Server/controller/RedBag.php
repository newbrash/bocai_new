...
class RedBag
{
    ...
    public static function chatRoomRedbag($data)
    {
        ...
        Redis::lpush(['grab_red_envelope_list'=>json_encode($data)]);
        // // DB::startTrans();
        // try {
        //     // redis索引
    ...
    public static function getSendRedList($data)
    {
        ...
        if (!$data['page']) {
            ...
            ->where('send_time', 'between', [$startTime, $endTime])
            ->count();
            // $sql = DB::name('user_red')->where('send_id', $data['send_id'])
            // ->where('send_time', 'between', [$startTime, $endTime])->fetchSql(true)
            // ->count();
            // dump($sql);
            // 统计发了多少金币
        ...
        try {
            ...
            if ($sendUserInfo['gold'] < $data['gold']) {
                return [
                    'status'=>false,
                    'code'=>101,
                    'errMsg'=>'当前金币为'.$sendUserInfo['gold'].'，不足以支付红包金额',
                ];
            }
            // 红包记录
            $redbagrecord = [
            ...
            if ($data['redType'] == 1) {
                ...
                $content = '(@title:金币扣除通知@)您于'.date('Y-m-d H:i:s').'发送一个金额为'.$data['gold'].'的个人红包！';
            } else {
                // 群发红包
                $msgData = [
                    'id'=>'',
                ...
                $userInfo = null;
                $content = '(@title:金币扣除通知@)您于'.date('Y-m-d H:i:s').'发送一个金额为'.$data['gold'].'的群发红包！';
            }
            // return $msgData;
            // return EXPIRE_TIME;
            $redbagInfo = [
