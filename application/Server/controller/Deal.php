...
class Deal
{
    ...
    public static function sendRedBagToFriend($connection, $data, $userCons)
    {
        ...
        if ($result['status']) {
            if ($data['redType'] == 1) {
                // 发个人红包
                $connection->send(json_encode([
                    'type'=>'responseSendRedbagToFriend',
                    'status'=>true,
            ...
            foreach ($friendIds['ids'] as $key => $vo) {
                foreach ($vo as $k => $v) {
                    ...
                    if (isset($userCons[$v['connectionID']]['uid']) && $userCons[$v['connectionID']]['uid'] == $v['id']) {
                        $noticeInfo = [
                            'type'=>'onlineNotice',
                            'msg'=>$userCons[$connection->id]['nickname'].'已经下线',
                            'fType'=> $key,
                            'index'=>$data['send_id'],
                            'status'=>0,
                            'connectionID'=>'',
    ...
    */
    ...
    public static function responseChatRoomRedbag($data, $userCons)
    {
        // dump($data);
        if (!$data['con_id'] || !isset($userCons[$data['con_id']]['uid']) || $userCons[$data['con_id']]['uid'] != $data['uid']) {
            $con_id = DataProcer::getUserConnectionID($data['uid']);
        } else {
            $con_id = $data['con_id'];
        }
        if ($con_id) {
            $userCons[$con_id]['con']->send(json_encode($data));
        }
    }
}
