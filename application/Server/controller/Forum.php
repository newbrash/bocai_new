...
class Forum
{
    ...
    public static function postTheme($data)
    {
        ...
        if ($id = DB::name('forum_post')->insertGetId($insertValue)) {
            ...
            file_put_contents('./rules.json', json_encode($rules));
            file_put_contents('./data.json', json_encode($data));

            if ($data['tieNum'] < $rules['num']) {
                ...
                $goldDetail = [
                    '奖励前'=>$userData['gold'],
                    '奖励金额'=>$rules['gold'],
                    '奖励后'=>$rules['gold'] + $userData['gold']
                ];
                $sliverDetail = [
                    '奖励前'=>$userData['integral'],
                    '奖励积分'=>$rules['sliver'],
                    '奖励后'=>$rules['sliver'] + $userData['integral']
                ];
                $userData['gold'] += $rules['gold'];
                $userData['integral'] += $rules['sliver'];
                if (DB::name('user_user')->update($userData)) {
                    ...
                    $systemNotice = [
                        ...
                        'time'=>time(),
                        'status'=>-1,
                        'content'=>'(@title:奖励通知@)您于'.date('Y-m-d H:i:s').'发布帖子《'.$data['title'].'》奖励金币'.$rules['gold'].'个和奖励积分'.$rules['sliver'].'积分，已发放至您的账户。',
                        'flag'=>0,
                    ];
                    $nid = DB::name('user_message')->insertGetId($systemNotice);
                    $systemNotice['id'] = $nid;
                    return [
    ...
    public static function themeComment($data)
    {
        ...
        $has = DB::name('forum_comment')->where(['user_id'=>$data['send_id'],'post_id'=>$data['post_id']])->value('id');
        if ($id = DB::name('forum_comment')->insertGetId($saveData)) {
            DB::name('forum_post')->where('id', $data['post_id'])->setInc('reply');
            if (!$has) {
                // 在当前帖子下第一次回帖
                $count = DB::name('forum_comment')->where('user_id', $data['send_id'])->group('post_id')->count();
                // dump("++++++++++++++++++++++++++++++++++");
                // dump($count);
                // dump("++++++++++++++++++++++++++++++++++");
                if ($count <= $rules['huitie_num']) {
                    // 回帖数小于或等于奖励回帖数
                    $updateData = DB::name('user_user')->where('id', $data['send_id'])->field('id,username,phonenumber,gold,integral,vip_time')->find();
                    
                    if ($saveData['addtime'] < $updateData['vip_time'] && ($rules['huitie_vip_sliver'] || $rules['huitie_vip_gold'])) {
                        $prideGold = $rules['huitie_gold']+$rules['huitie_vip_gold'];
                        $prideIntegral = $rules['huitie_sliver']+$rules['huitie_vip_sliver'];
                        // 当前用户为vip用户并且设置了vip额外奖励
                        $goldHistrory = [
                            '奖励前'=>$updateData['gold'],
                            '奖励金额'=>$rules['huitie_gold'],
                            'vip奖励金额'=>$rules['huitie_vip_gold'],
                            '奖励后'=>$updateData['gold']+$prideGold,
                        ];
                        $sliverHistory = [
                            '奖励前'=>$updateData['integral'],
                            '奖励积分'=>$rules['huitie_sliver'],
                            'vip奖励积分'=>$rules['huitie_vip_sliver'],
                            '奖励后'=>$updateData['integral']+$prideIntegral,
                        ];
                        $noticeContent = '(@title:奖励通知@)您于'.date('Y-m-d H:i:s').'回复贴子《'.$data['post_title'].'》获得奖励金币'.$prideGold.'个(基本奖励'.$rules['huitie_gold'].',vip额外奖励'.$rules['huitie_vip_gold'].')和奖励积分'.$prideIntegral.'积分(基本奖励'.$rules['huitie_sliver'].',vip额外奖励'.$rules['huitie_vip_sliver'].')，已发放至您的账户。';
                    } else {
                        $prideGold = $rules['huitie_gold'];
                        $prideIntegral = $rules['huitie_sliver'];
                        // 非vip奖励
                        $goldHistrory = [
                            '奖励前'=>$updateData['gold'],
                            '奖励金额'=>$rules['huitie_gold'],
                            '奖励后'=>$rules['huitie_gold']+$updateData['gold'],
                        ];
                        $sliverHistory = [
                            '奖励前'=>$updateData['integral'],
                            '奖励积分'=>$rules['huitie_sliver'],
                            '奖励后'=>$updateData['integral']+$rules['huitie_sliver'],
                        ];
                        $noticeContent = '(@title:奖励通知@)您于'.date('Y-m-d H:i:s').'回复贴子《'.$data['post_title'].'》获得奖励金币'.$prideGold.'个和奖励积分'.$prideIntegral.'积分，已发放至您的账户。';
                    }
                    $updateData['gold'] += $prideGold;
                    $updateData['integral'] += $prideIntegral;
                    if (DB::name('user_user')->update($updateData)) {
                        // 保存金币流水
                        Base::goldHistory($updateData['username']?$updateData['username']:$updateData['phonenumber'], '发帖奖励', $goldHistrory);
                        // 保存积分流水
                        Base::sliverHistory($updateData['username']?$updateData['username']:$updateData['phonenumber'], '发帖奖励', $sliverHistory);
                        // 检查和更新用户的勋章信息
