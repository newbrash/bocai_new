...
class Gameapi
{
    ...
    */
    ...
    public static function gameData_1($game, $row = 5, $type = 'json')
    {
        try {
            $url = "http://f.apiplus.net/$game-$row.$type";
            // dump($url);
            $data = self::httpRequest($url, 15);
            if ($type == 'json') {
            ...
            if (!$data || $data==null || !is_array($data)) {
                throw new \Exception("gameapi error: get data from url($url) failed!");
            }
        } catch (\Exception $e) {
            echo $e->getMessage()." try another gameapi to get game data!\n";
            try {
                $url = "http://www.caipiaojieguo.com/api/lottrey?biaoshi=".$game."&format=".$type."&rows=".$row;
                $data = self::httpRequest($url, 15);
                if ($type == 'json') {
                    $data = json_decode($data, true);
                } else {
                ...
                echo "gameapi error:".$e->getMessage()." in file:".$e->getFile()." at line:".$e->getLine()."\n";
            }
        }
        $data = $data['data'];
        if (isset($data[0]['qishu'])) {
            $temp = [];
            foreach ($data as $key => $vo) {
                $temp[$key]['expect'] = $vo['qishu'];
                $temp[$key]['opentime'] = $vo['open_time'];
                $temp[$key]['opentimestamp'] = strtotime($vo['open_time']);
                $temp[$key]['opencode'] = $vo['result'];
            }
            $data = $temp;
        }
        if ($row == 1) {
