...
public function caiji(){
    ...
    public function diceResult(){
        ...
        *'opentime' => string '2018-10-16 21:35:05' (length=19)//开间时间
        ...
        try{
            $dice = new Gameapi();
            $this->dice = $dice->gameData_1('xglhc');
            $data = $this->dice['data']['data'][0];
            $dice = explode(",", $this->dice['data']['data'][0]['opencode']);
        }
        catch(\Exception $e){
            echo Deal::iconv("六合彩接口采集数据失败，此处为测试数据\n");
            //测试
            $flag = 0;
            $opencode = [];
            do{ 
    ...
    */
    ...
    public function recentOpen($yema){
        $recentOpen = Db::name('games_six_dice_history')->field('issue,open_time,code')->limit($yema*15,15)->order('id desc')->select();
        return $recentOpen;
    }
    /*
