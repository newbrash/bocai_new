...
use app\GameServer1\controller\Server1 as MyServer;
...
class ServerCommand extends Command
{
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        // 输出描述信息
        $output->writeln('Game Server1 is opening...');
        // 获取worker配置信息
        $option = Config::pull('worker');

        $host = !empty($option['host']) ? $option['host'] : '0.0.0.0';
        $port = !empty($option['gameport']) ? $option['gameport'] : 2346;
        // 没弄懂。。。待续
