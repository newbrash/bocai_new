...
class Index extends Controller
{
    ...
    {
        //************前台判断是否足够玩家，是否全部押注 是否全部准备*********************************
        $request = Request::param();
        if ($request) {
            //玩家进入游戏房间，放回新手，高手，大神牌桌的信息(最低金币以及牌桌人数)
            $enter = self::enterRoom($request['id']);
            if ($enter) {
                echo '{"code":0}';
            } else {
                echo '{"code":1}';
        ...
        //预先把所有在线用户的信息放到桌面上
       // $player = Db::name('user_user')->where('ips','>','0')->select();
    ...
    */
    ...
    {
        // try{
        //     $roomInfo['gold_min'] = Db::name("games_sg_desk_setting")->column("gold_min");
        //     $roomInfo['gold_max'] = Db::name("games_sg_desk_setting")->column("gold_max");
        //     $roomInfo['online_setting'] = Db::name("games_sg_desk_setting")->column("online_setting");
        //     $roomInfo['desk_id'] = Db::name("games_sg_desk_setting")->column("desk_id");
        //     $roomInfo['multiple']= Db::name("games_sg_desk_setting")->column("multiple");
        // }
        // catch(Exception $e){
    ...
    // 用户已经开始界面    //bet就是workman传过来的数据
    // public function begin()
    public function begin($bet)
    {
        $player['data'] = $bet;
        echo "game begin";
        /*游戏开始后 接受玩家的押注信息
        ** gold status:0 对牌 1三公 2赢 3和 4输
        */
        /*庄家不能押注
        * 庄家的信息要返回去
        * 每局游戏结束的时候要产生新的庄家
        */
        // 庄家逃跑或者掉线，庄家不变
        // file_put_contents("./player.json", json_encode($player['data'],true));
        $banker = Db::name('games_sg_banker')->alias('id')->order('time desc')->join('user_user u', 'u.id = id.banker_id')->field('u.gold,u.id,u.bocai')->find();
        ...
        if ($banker['bocai']) {
            ...
            $orderSub = self::order();
            //发牌
            $deal = $power->bocaiDeal($orderSub, $power);
        }
        //非平台庄家发牌
        else {
            //产生发牌顺序
        ...
        foreach ($player['data'] as $key => $vo) {
            ...
            $vo['remaining'] = $player['data'][$key]['gameBefore'] - $vo['allGold'];
            Db::name("user_user")->where("id", $vo['id'])->update(['gold'=>$vo['remaining']]);
            //庄家不需要参加游戏结算
            if ($player['data'][$key]['player']!='banker') {
                $result[$key]  = self::gameResult($vo, $deal, $odds);
            }
        }
        ...
        if ($banker['gold'] >= $total) {
            foreach ($player['data'] as $key => $vo) {
                if ($vo['player'] != "banker") {
                    $player['data'][$key]['result'] = $result[$key];
                    $player['data'][$key]['gameAfter'] =  $player['data'][$key]['remaining'] + $player['data'][$key]['result'];
                }
            }
        ...
        // echo "banker['gold']:";dump($banker['gold']);
            ...
            foreach ($player['data'] as $key => $vo) {
                if ($vo['player'] != "banker") {
                    $player['data'][$key]['result'] = $result[$key]*($banker['gold']/$total);
                    $player['data'][$key]['gameAfter'] =  $player['data'][$key]['remaining'] + $player['data'][$key]['result'];
                }
            }
            $banker['gold'] = 0;
        ...
        foreach ($player['data'] as $key => $vo) {
            if ($vo['player']=='banker') {
                ...
                $player['data'][$key]['orderSub'] = $orderSub;//发牌顺序
                    ...
                    $player['data'][$key]['result'] = $banker['gameAfter'];//用户剩余金币
            ...
            // $player['data'][$key] = json_encode($player['data'][$key]);
                ...
                $player['data'][$key]['code'] = 5;//发牌顺序
                    $player['data'][$key]['orderSub'] = $orderSub;//发牌顺序
                    $player['data'][$key]['deal'] = $deal;   //发牌结果
                    $player['data'][$key]['result'] =$result[$key];//用户剩余金币
                    $player['data'][$key]['sub'] = $sub;
    ...
    */
    ...
    public function record($banker, $player, $deal)
    {
        ...
        foreach ($player['data'] as $key=>$vo) {
            if ($vo['player']!="banker") {
                ...
                $new = $gold + $player['data'][$key]['result'];
                Db::name('user_user')
    ...
    {
        $pokerOrder = config("template.tpl_replace_string.__WWW__").'/bc/public/SanGong/poker/wintersweet/'.$orderSub.'.png';
        return $pokerOrder;
    }
    //把牌组的数据转换成扑克牌
    ...
    public function poker($deal)
    {
        ...
        foreach ($deal as $key =>$vo) {
            ...
            foreach ($vo as $keysub=>$vo) {
                //$deal['banker'][0]=config("template.tpl_replace_string.__WWW__").'/bc/public/SanGong/poker/wintersweet/'.$vo['color'].'/'.$vo['size'].'.png'
                $deal[$key][$keysub] = config("template.tpl_replace_string.__WWW__").'/bc/public/SanGong/poker/'.$vo['color'].'/'.$vo['size'].'.png';
            }
        }
        return $deal;
    }
    ...
    */
    public function enterDesk($data = [])
    {
        ...
        //用户达到了进入房间的条件，返回页面初始化数据

        else {
            //赔率
            $percent = Db::name('games_sg')->field('type,odds,result')->select();
            $stemp = [];
    ...
    //获取游戏结果
    public function gameResult($request, $deal, $odds)
    {
        $allResult = 0;
        ...
        foreach ($request['bet'] as $key=>$vo) {
            if (in_array($vo['status'], $classifyBanker)) {
                $result = self::classifyBanker($request, $vo['status'], $deal[$vo['player']], $deal['banker'], $odds[$vo['player']]);
            } else {
