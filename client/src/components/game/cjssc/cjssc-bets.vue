...
export default {
  ...
  computed: {
    ...
    nwhShouldRender() {
      ...
      let temp = [
        ...
        [
          /^wuxin-zhixuan|^sixin-qiansizhixuan-(fushi|zuhe)$|renxuan-rener-fushi$|dindanwei/,
          [0, 1, 2, 3, 4]
        ], //显示 万千百十个 五星直选 复式 组合, 四星复式 组合, 任二 复式, 定位胆
  ...
  methods: {
    ...
    nwhName(nwhindex) {
      ...
      if (
        new RegExp(
          /^sixin\S+(?=zuxuan12)|^wuxin-wuxinzuxuan-zuxuan(60|30)$/
        ).test(currentOdds)
      ) {
        // 四星组选12 五星组选60 30
        nwhName = { 0: "二重号", 1: "单号" };
      ...
      if (
        new RegExp(/^sixin\S+(?=zuxuan4)|^wuxin-wuxinzuxuan-zuxuan20$/).test(
          currentOdds
        )
      ) {
        // 四星组选4 星组选20
  ...
  watch: {
    ...
    currentOdds: {
      handler(currentOdds) {
        ...
        let numWithHeader = new RegExp(
          /^(sixin|wuxin)\S+(?<!danshi)$|^renxuan\S+(?<!danshi|hunhezuxuan)$|^(erxin|sanxin)\S+(?=fushi|zhixuankuadu)|^sanxin\S+(?=zusan|zuliu)|(budinwei|qvwei|dindanwei)/
        );

        //显示 玩法名称和文字球
        //大小单式所有玩法  daxiaodanshuang
        //二星 三星 名字带组选包胆的玩法 ^(erxin|sanxin)\S+(?=zuxuanbaodan)
        let showBallText = new RegExp(
          /^daxiaodanshuang|^(erxin|sanxin)\S+(?=zuxuanbaodan)/
        );
        ...
        let showBallNum = new RegExp(
          /^(erxin|sanxin)\S+(?=zhixuanhezhi|heshiweishu)/
        );
