<template>
  <div class="contanier">
          ...
          <tbody v-if="recentInfo.length">
            <tr v-for="(tr,index) in recentInfo"
              ...
              <td class="td-dice-wrap flex justify-sa">
                <span v-for="n in tr.opencode"
                      class="open-num-ball">{{n}}</span>
              </td>
              <td>{{tr.sum}}</td>
              <td>{{tr.maxOrMin}}</td>
              <td>{{tr.doubleOrFloat}}</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <!-- 玩法规则弹窗 -->
    <dentGameRuleModal ruleKey="cjssc-rules"
    ...
    shouldCommit: {
      handler(shouldCommit) {
        ...
        const switchFunc = [
          ...
          ["^wuxin-wuxinzuxuan-zuxuan10$", 五星组选10], //五星 组选10
          ["^wuxin-wuxinzuxuan-zuxuan5$", 五星组选5], //五星 组选5
          ["^wuxin-zhixuan-danshi$", 五星单式], //五星 单式
        ...
        switchFunc.every(arrayLike => {
          if (new RegExp(arrayLike[0]).test(currentOdds)) {
            let ifNeedBreak =
              arrayLike[1].call(this, arrayLike[2]) === undefined
                ? false
                : true;
            return ifNeedBreak;
          } else {
        ...
        function 五星组选60() {
          let code2 = shouldCommit[0]; //二重号
          let singleNum = shouldCommit[1]; //单号
          //数组去重
          let unpLength = Array.from(new Set([...code2, ...singleNum])).length;

          //相同的个数
          let code2Length = code2.length;
          let singleNumLength = singleNum.length;
          let dupLength = code2Length + singleNumLength - unpLength;

          if (unpLength >= 3) {
            //总注数
            totalBets = code2Length * calcBets(singleNumLength, 3);
            if (dupLength) {
              // 计算一组（二重号和单号相同）的注数
              totalBets -= dupLength * calcBets(singleNumLength - 1, 2);
            }
          }
        }

        function 五星组选30() {
          let code2 = shouldCommit[0].length; //二重号
          let singleNum = shouldCommit[1].length; //单号
          //数组去重
          let unpLength = Array.from(
            new Set([...shouldCommit[0], ...shouldCommit[1]])
          ).length;
          //相同的个数
          let dupLength = code2 + singleNum - unpLength;
          if (unpLength >= 3) {
            totalBets = singleNum * calcBets(code2, 2);
            totalBets -= dupLength * calcBets(code2 - 1, 1);
          }
        }

        function 五星组选20() {
          let code3 = shouldCommit[0].length; //三重号
          let singleNum = shouldCommit[1].length; //单号
          //数组去重
          let unpLength = Array.from(
            new Set([...shouldCommit[0], ...shouldCommit[1]])
          ...
          //相同的个数
          let dupLength = code3 + singleNum - unpLength;
          if (unpLength >= 3) {
            totalBets = code3 * calcBets(singleNum, 2);
            totalBets -= dupLength * calcBets(singleNum - 1, 1);
          }
        }

        function 五星组选10() {
          let code2 = shouldCommit[0].length; //二重号
          let code3 = shouldCommit[1].length; //三重号
          //数组去重
          let unpLength = Array.from(
            new Set([...shouldCommit[0], ...shouldCommit[1]])
          ).length;
          //相同的个数
          let dupLength = code3 + code2 - unpLength;

          if (code2 && code3) {
            totalBets = code2 * code3 - dupLength;
          }
        }

        function 五星组选5() {
          let code4 = shouldCommit[0].length; //四重号
          let singleNum = shouldCommit[1].length; //单号
          //数组去重
          let unpLength = Array.from(
            new Set([...shouldCommit[0], ...shouldCommit[1]])
          ...
          //相同的个数
          let dupLength = singleNum + code4 - unpLength;

          if (code4 && singleNum) {
            totalBets = code4 * singleNum - dupLength;
          }
        }
