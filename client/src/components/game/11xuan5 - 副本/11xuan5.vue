<template>
  <div class="contanier">
    ...
    <!-- 固定头部 -->
    <dentGameHeader v-on:onSubSelect="mixins_onSwitch" v-on:onClickOpr="mixins_onClickOpr" :rightOrder="haveGLOBALOrderList" v-on:onClickRightOrder="mixins_goTOPay(false)" :obj="this.GLOBAL.odds" :gameKey="gameKey" subType="multipleSub" rightNormal="true">
    ...
    <div class="flex-wrap flex flow-col">
      <div class="open flex bgc-fff">
        ...
        <div class="time-left flex flow-col justify-sa">
          ...
          <div style="letter-spacing: .1em;" class="font-theme">
            {{timer.timeToOpen}}
          </div>
        </div>
        <!-- 开奖号码 -->
        <div v-if="recentInfo.length" @click="openLately=!openLately" dentHoverclass="hoverclass" class="open-num flex flow-col justify-sa">
          <p class="flex">第{{recentInfo[0].expect}}期 <span :class="openLately?'_arrow-down':''" class="iconfont icon-arrow-down"></span> </p>
          <div class="flex justify-ct">
            <span v-for="n in ['07','06','10','01','11']" class="open-num-ball">{{n}}</span>
          </div>
        </div>
        <div v-else class="connecting flex flow-col justify-sa">
          <span>正在连接服务器,请不要下单</span>
          <mt-spinner color="#e62b00" type="fading-circle"></mt-spinner>
        </div>
      </div>
      <!-- 近期开奖 -->
      <div :class="openLately?'open-lately-expand':''" class="open-lately">
        <table>
          <colgroup>
            <col width='20%'>
            <col width='50%'>
          ...
          <tbody v-if="recentInfo.length">
            <tr v-for="(tr,index) in recentInfo" :class="index%2===1?'bg-grey':''">
              <td>{{tr.expect}}</td>
              <td class="td-dice-wrap flex justify-sa">
                <span v-for="n in tr.open_code" class="open-num-ball">{{n}}</span>
              </td>
              <td>{{tr.sum}}</td>
              <td>{{tr.kuadu}}</td>
            </tr>
      ...
      <div class="odds-wrap flex flow-col">
        <bets v-model="shouldCommit" :betsObj="betsObj" :currentOdds="currentOdds" v-on:ruleModalShow="ruleModalShow=true"></bets>
        <!-- 手动选号符合规则的注单 -->
        <transition name="popUp">
          <div v-if="handy" v-show="totalBets" class="bets-handy">
            <transition-group name="betsDropin" tag="div">
              <div v-for="(n,index) in shouldCommit" :key="index" class="bets-handy-item">
        ...
        <transition name="slideUp">
          <div v-show="totalBets" class="bets-total flex justify-sb">
            <div>
      ...
      <div class="hecai-bottom flex justify-sb bgc-fff">
        <div @click="shouldCommit = []" class="hecai-btn random-btn">清空</div>

        <div class="bets-per-gold">单注<input v-model="default_gold" type="text">元</div>

        <div v-if="totalBets" @click="sendOrderToPage" class="hecai-btn submit-btn cansubmit">确定</div>
        <div v-else class="hecai-btn submit-btn">确定</div>
      </div>

    </div>

    <!-- 玩法规则弹窗 -->
    <dentGameRuleModal ruleKey="gd-rules" v-model="ruleModalShow" :currentOdds='currentOdds'></dentGameRuleModal>
...
export default {
  ...
  mounted() {
    this.mixins_getOddsData({
      gameKey: "Gd11x5",
      断网了: false,
      request_type: "getOdds",
      type: "getInfos"
    });
  },
  methods: {
    ...
    shouldCommit: { 
      handler(shouldCommit) {
        ...
        function calcBets(bets, n) {
          ...
          let totalBets =
            Factorial(bets) / (Factorial(bets - n) * Factorial(n));
          return totalBets;
        }
