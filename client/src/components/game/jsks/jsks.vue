...
export default {
  ...
  computed: {
    ...
    betsObj() {
      let betsObj;
      let ifNeedBetsObj = ["sum-sum-size", "sum-sum-sum"].includes(
        this.currentOdds
      );
      if (ifNeedBetsObj) {
  ...
  watch: {
    ...
    shouldCommit: {
      handler(shouldCommit) {
        ...
        function getMaxOdds() {
          if (
            ["twoSame-twoSameSg-commom", "twoSame-twoSameSg-handy"].includes(
              this.currentOdds
            )
          ) {
            //二同号单选的 标准选号 和 手动选号
            ...
            ["sum-sum-size", "sum-sum-sum"].includes(this.currentOdds)
          ) {
            //和值 大小单双 和 和值
