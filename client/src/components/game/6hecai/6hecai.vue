...
export default {
  ...
  methods: {
    ...
    sendOrderToPage() {
      //先判断是否登录
      this.checkLogin(() => {
        let id = this.GLOBAL.userInfo.id;
        ...
        if (currentOdds === "hq") {
          ...
          let leiXin = shengxiaoOrNot
            ? shengxiao + shouldCommit.length
            : "else" + shouldCommit.length;
          order.leiXin = leiXin;
    ...
    shouldCommit: {
      handler(shouldCommitVal) {
        ...
        function defaultFunc() {
          ...
          let obj = currentSubOdds
            ? odds[currentOdds][currentSubOdds]
            : odds[currentOdds];

          maxOdds = Math.max(
            ...shouldCommitVal.map(key => {
              return obj[key];
            })
          );
        ...
        function minSelectFunc() {
          ...
          let minLengtn = currentSubOdds.match(/\d+/)[0]; //从玩法名字中截取数字
          if (shouldCommitVal.length < minLengtn) {
            return;
          }

          //最多可以选 maxLength 个号码
          let defaultLength = 10;
          let temp = { 8: 11, 9: 12, 10: 13, 11: 13, 12: 14 };
          let maxLength = temp[minLengtn] || defaultLength;

          if (shouldCommitVal.length > maxLength) {
            Toast({
              message: "最多只能选择" + maxLength + "个球",
              duration: 2000
            });
            shouldCommitVal.pop();
            return;
        ...
        function zhmggFunc() {
          ...
          maxOdds = 1;
          for (let subkey in shouldCommitVal) {
            //正码过关 赔率乘起来
        ...
        function hqFunc() {
          ...
          let maxOddsKey = shengxiaoOrNot
            ? shengxiao + shouldCommitVal.length
            : "else" + shouldCommitVal.length;

          totalBets = 1;
          maxOdds = shengxiaoOrNot
            ? odds.hq.odds.fensang[shengxiao][maxOddsKey]
            : odds.hq.odds.fensang.else[maxOddsKey];
          this.maxOdds = maxOdds;
        }
