  ...
  <div class="contanier flex flow-col">
    ...
    <header class="dent-header flex justify-sb fz-16">
      <div
        ...
        @click="$router.go(-1)"
        class="isleft flex iconfont icon-left click-icon"
      ></div>
    ...
    <switchTab :tabObj="tabObj" flex="true" v-model="activeTab"></switchTab>

    <mt-tab-container swipeable="true" v-model="activeTab">
      <mt-tab-container-item v-for="(value,key,index) in tabObj" :id="index" :key="index">
        <!-- 全部注单                               -->
        <template v-if="key ===  '全部注单'">
          <template v-if="allOrder.length">
            <div style="height: calc(100vh - 94px); overflow: auto;" id="loadBox">
              <div
                v-for="order in allOrder"
              ...
              >
                ...
                <div class="order-status flex justify-sb fz-14">
                  <span>投注: {{order.gold.split(/\s+/)[1]}}</span>
                  <span v-if="order.status === 0" class="flex">
                    <b class="font-wait-color iconfont icon-wait"></b>待开奖
                  </span>
                  <span v-if="order.status === 1" class="flex fontc-9">
                    <b class="iconfont icon-zhongjiang"></b>没有中奖
                  </span>
                  <span v-if="order.status === 2" class="flex font-theme">
                    <b class="iconfont icon-zhongjiang"></b>
                    中奖{{order.win.toFixed(3)}}
                  </span>
                  <span v-if="order.status === 3" class="flex fontc-9">
                    <b class="iconfont icon-close"></b>已撤单
                  </span>
                </div>
              </div>
            </div>
          ...
          <!-- loading提示 -->
          <div v-else class="order-empty flex justify-ct flow-col">
            ...
            <mt-spinner v-else color="#e62b00" type="fading-circle"></mt-spinner>
          </div>
        </template>
        <!-- 待开奖订单 -->
    ...
    <transition name="orderEnter">
      <div v-show="orderDetailShow" class="order-detail flex flow-col">
        <header class="dent-header flex justify-sb fz-16">
          <div
            dentHoverclass="hoverclass"
          ...
          <div class="order-detail-info flex flow-col justify-sa">
            <div class="flex flow-col">
              <div class="game-avatar">
                <img :src="currentOrder.avatar">
              </div>
              <div class="game-name fz-16">{{currentOrder.gameName}}</div>
            </div>
            <div class="game-issue fz-12 fontc-9">第{{currentOrder.issue}}期</div>
            <div v-if="currentOrder.status === 0" class="game-status font-theme">待开奖 ... ...</div>
            <div v-if="currentOrder.status === 1" class="game-status fontc-6">没有中奖，再接再厉</div>
            <div
              v-if="currentOrder.status === 2"
              class="game-status font-theme"
            >中奖 {{currentOrder.win.toFixed(3)}} 元！</div>
            <div v-if="currentOrder.status === 3" class="game-status fontc-6">该订单已撤销</div>
          </div>
          <!-- 订单详细 -->
          <div class="order-detail-list">
            <div class="flex order-detail-item">
              <span class="fz-12 fontc-9">开奖号码</span>
              <p v-if="currentOrder.status === 0" class="fz-14 fontc-6">未开将</p>
              <p v-else class="fz-14 fontc-6">{{currentOrder.open_code}}</p>
            </div>
            <div class="flex order-detail-item">
              <span class="fz-12 fontc-9">投注号码</span>
              <p v-if="openNumArray" class="fz-14 fontc-6">
            ...
            <div class="flex order-detail-item">
              ...
              <p
                class="fz-14 fontc-6"
              >{{ splitCurrentOrderName(currentOrder.name, currentOrder.key) }}</p>
            </div>
            <div class="flex order-detail-item">
        ...
        <span
          ...
          class="iconfont icon-refund icon-refund-pos"
        ></span>
        <!-- 未中奖 -->
        <span
          v-if="currentOrder.status === 1"
          class="iconfont icon-weizhongjiang icon-weizhongjiang-pos"
        ></span>
        <!-- 中奖 -->
        <span
          v-if="currentOrder.status === 2"
          class="iconfont icon-zhongjiangliao icon-zhongjiangliao-pos"
        ></span>

        <!-- 再来一注 -->
        <div @click="$router.go(-1)" class="buymore">再来一注</div>
      </div>
    </transition>
  </div>
</template>
...
export default {
  ...
  computed: {
    //投注的号码
    openNumArray() {
      if (typeof this.currentOrder.number == "object") {
  ...
  methods: {
    ...
    //发起请求
    getOddsData(page = 0) {
      let user_id = this.GLOBAL.userInfo.id;
      if (user_id) {
        this.senddata({
          data: {
            type: "betHistory",
    ...
    splitCurrentOrderName(value, gameKey) {
      ...
      if (neeDsplit) {
        ...
        let newArray = array.map(item =>
          this.mixin_KeyToCharacter(item, gameKey)
        );
