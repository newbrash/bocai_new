...
new Vue({
  ...
  created: function() {
    ...
    //判断参数的类型 this.$check( '123' , ['string','number'] ) === true
    Vue.prototype.$check = function(needCheck, type) {
      function formate(parm) {
        return (
          parm.substring(0, 1).toUpperCase() + parm.substring(1).toLowerCase()
        );
      }
      ...
      if (typeof arguments[1] === "string") {
        ...
        return arguments[1].some(item => {
          item = formate(item);
          return (
            Object.prototype.toString.call(needCheck) === `[object ${item}]`
          );
        });
      }
    };
    ...
    Vue.prototype.$drawline = function({ selector, ballClass = ".openball" }) {
      ...
      function insert({ ballHeight }) {
        ...
        if (!canvas) {
          let canvas = `<canvas width="${dom.offsetWidth}" height="${
            ...
            dom.offsetLeft
          };top: ${dom.offsetTop + ballHeight}px; z-index: 1;"></canvas>`;
      ...
      //开始画线
      function stroke({ points, ballWidth, ballHeight }) {
        let canvas = dom.parentNode.querySelector("canvas");
