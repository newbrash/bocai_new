...
const gameFunc = {
  ...
  methods: {
    ...
    dealOnMessage() {
      ...
      this.onmessage(res => {
        ...
        //最新期号
        if (res.issue) {
          this.GLOBAL[gameKey].nextIssue = this.nextIssue = res.issue;
        }
        //处理赔率信息
    ...
    mixins_onSwitch(val) {
      ...
      this.shouldCommit = "reset"; //通知子组件重置订单信息
    },
  ...
  beforeDestroy() {
    //重置游戏的全局变量
    this.GLOBAL[this.gameKey] = JSON.parse(
      JSON.stringify(this.GLOBAL.template)
    );
  }
};
...
const filterMethods = {
  methods: {
    mixin_KeyToCharacter(value, gamekey) {
      ...
      let matchKey = gamekey || this.gameKey; //之所以有这个判断，是因为游戏的购彩记录页，会有多个游戏的注单,可能要传入不同的游戏gamekey
      //数字不用匹配直接返回
      if (!isNaN(value)) {
        return value;
      }
      //没有匹配到 或者没有传入
      if (keyJSON[matchKey] === undefined || value === undefined) return value;
