exports.install = function (Vue, options) {
    ...
    Vue.prototype.gameHandle = function (callback) {
        if (this.GLOBAL.gameConnectionId) {
            if (callback) {
                callback();
            }
        } else {
            if (callback) {
                gameHandlecallbck = callback;
            }
            let data = {
                type: 'handle',
                send_id: this.GLOBAL.userInfo.id ? this.GLOBAL.userInfo.id : false,
                nickname: this.GLOBAL.userInfo.nickname ? this.GLOBAL.userInfo.nickname : false,
            }
            this.senddata({ data: data, handType: 'game' });
        }
    }
    /**
     *  动态添加属性
    ...
    Vue.prototype.onmessage = function (onmessagecallback) {
        ...
        this.GLOBAL.socketHand.onclose = function (evt) {
            console.log("user socket is closing...");
        }
        if (this.GLOBAL.gameOrNot) {
            // 游戏socket接收消息处理
            this.GLOBAL.gameSocketHand.onmessage = function (evt) {
                ...
                if (evt.data) {
                    try {
                        ...
                        switch (data.type) {
                            case "handle":
                                that.GLOBAL.gameConnectionId = data.connectionId;
                                console.log(gameHandlecallbck);
                                if (gameHandlecallbck) {
                                    gameHandlecallbck();
                                    gameHandlecallbck = null;
                                }
                                break;
                            case "heart":
                                that.senddata({ data: { type: 'heart', text: 'pong' } });
                                break;
                        }
                    } catch (e) {
                        // data = null;
                ...
                let senddataParam = data ? data : evt.data;
                if (onmessagecallback) {
                    onmessagecallback(senddataParam);
                }
            ...
            this.GLOBAL.gameSocketHand.onclose = function (evt) {
                console.log("game socket is closing...");
                that.GLOBAL.gameSocketHand = null;
                that.GLOBAL.gameConnectionId = null;
        ...
        if (evt.data) {
            ...
            switch (data.type) {
                ...
                case "updateUserGold":
                    ...
                    break;
                case "responseChatRoomRedbag":
                    responseChatRoomRedbag(data, that);
                    break;
                default:
            }
    ...
    function responseChatRoomRedbag(data, that) {
        console.log("++++++++++++++++++++");
        console.log(data);
        console.log("++++++++++++++++++++");
        that.$forceUpdate();
        if (data.status) {
            ...
            localStorage.setItem(
                "bc_userInfo",
                that.GLOBAL.ObjToString(that.GLOBAL.userInfo)
            );
            // 更新本地红包信息的收取状态
            that.GLOBAL.chatRoomMsgList[data.msg_index]["content"] =
                data.content;

            // 隐藏红包弹窗
            that.GLOBAL.chatRoomShowRedbag = false;
            // 跳转红包详情
            that.$router.push({
                path: "/redbagdetail",
                query: { red_id: data.rid, chatRoom: true }
            });
        } else {
            // 输出提示
            that.$messagebox.alert(data.msg).then(action => {
                that.$forceUpdate();
                that.GLOBAL.chatRoomShowRedbag = false;
                console.log("show:" + that.GLOBAL.chatRoomShowRedbag);
            });
            try {
                ...
                console.log(that.GLOBAL.chatRoomMsgList[data.msg_index].content);
                console.log("PPPPPPPPPPPPPPPPPPPPPPPPPPPPP");
            } catch (e) {
                console.log(e);
            }
    ...
    function updateUserGold(data, that) {
        ...
        that.updateSystemNotice(data.noticeMsg);
    ...
    function giveBackTransferNotice(data, that) {
        ...
        if (data.systemNotice) {
            that.updateSystemNotice(data.systemNotice);
        }
        if (data.fid) {
    ...
    Vue.prototype.updateSystemNotice = function ($notice) {
        if (this.GLOBAL.msgList.systemNotice) {
            this.$set(this.GLOBAL.msgList.systemNotice, this.GLOBAL.getObjLenght(this.GLOBAL.msgList.systemNotice), $notice);
        } else {
            this.$set(this.GLOBAL.msgList, 'systemNotice', {});
            this.$set(this.GLOBAL.msgList.systemNotice, 0, $notice);
        }
