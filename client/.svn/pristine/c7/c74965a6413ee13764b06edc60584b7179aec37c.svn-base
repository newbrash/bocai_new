...
export default {
  ...
  methods: {
    ...
    cancleImg(img, index) {
      ...
      var url = this.GLOBAL.Host + "/bcweb/index.php/Server/Uploads/unlinkFile";
      let that = this;
      this.$axios
        .get(url, {
          params: {
    ...
    addImgToComment() {
      ...
      var url =
        this.GLOBAL.Host + "/bcweb/index.php/Server/Uploads/uploadCommentImg";
    ...
    sendComment() {
      ...
      if (this.reply_index < 0) {
        ...
        let data = {
          ...
          con_id: this.GLOBAL.connectionId,
          imgs: this.imgsrcs,
          post_title: this.themeDetail.title
        };
        let that = this;
        this.senddata({
          data: data,
          callback: function(respone) {
            if (respone.status) {
              ...
              if (respone.systemNotice) {
                that.updateSystemNotice(respone.systemNotice);
              }
              if (respone.gold != -505 || respone.integral != -505) {
                that.GLOBAL.userInfo.gold = respone.gold;
                that.GLOBAL.userInfo.integral = respone.integral;
          ...
          if (respone.status) {
            ...
            if (!that.page) {
              ...
              if (respone.noticeMsg) {
                that.updateSystemNotice(respone.noticeMsg);
              }
              that.page++;
            }
          } else {
