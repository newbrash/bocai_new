  ...
  <div class="container">
    ...
    <div class="ex-header flex flow-col">
      ...
      <template v-if="currentTable=== '定位胆'">
        ...
        <!-- 球数 -->
        <div class="ball-wrap flex align-stretch">
          <label v-for="(value,key,index) in switchBall"
                 :class="site===value?'current-ball':''"
                 class="ball-item flex justify-ct">
            <input type="radio"
                   :value="value"
                   v-model="site"
                   class="dent-hide-input">
            <mt-spinner v-if="site===value && siteLoading"
                        color="#fff"
                        size="20"
      ...
      <template v-if="currentTable=== '冠亚和'">
        <div class="total flex justify-ct flow-col">
          <!-- 表格主体 -->
          <!--  -->
          <template v-if="gyh === 0 && gyhTableData[0] && gyhTableData[0].table.length">
            <!-- 固定表头 -->
            <table class="fixheader"
                   style="position: sticky;top: 0;z-index: 2;">
              <colgroup>
            ...
            <div class="table-main-wrap">
              ...
              <table class="canvas-table"
                ...
                <tbody>
                  <tr v-for="(tr,index) in gyhTableData[0].table"
                    ...
                    <td>{{tr.issue}}</td>
                    <td>{{tr.guanyahe}}</td>
                    <td>{{tr.heda | keyToCharacter}}</td>
                    <td>{{tr.hexiao | keyToCharacter}}</td>
                    <td>{{tr.hedan | keyToCharacter}}</td>
                    <td>{{tr.heshuang | keyToCharacter}}</td>
                  </tr>
                </tbody>
              </table>
              <!-- 固定表脚 -->
              <table style="position: sticky;bottom: 0;z-index: 1;">
                ...
                <tfoot class="bg-yellow">
                  <tr>
                    <td>出现次数</td>
                    <td v-for="value in  gyhTableData[0].analyse.times">
                      {{ value }}
                    </td>
                  </tr>
                  <tr>
                    <td>平均遗漏</td>
                    <td v-for="value in  gyhTableData[0].analyse.pinjunyilou">
                      {{ value }}
                    </td>
                  </tr>
                  <tr>
                    <td>最大遗漏</td>
                    <td v-for="value in  gyhTableData[0].analyse.zuidayilou">
                      {{ value }}
                    </td>
                  </tr>
                  <tr>
                    <td>最大连出</td>
                    <td v-for="value in  gyhTableData[0].analyse.zuidalianchu">
                      {{ value }}
                    </td>
                  </tr>
                </tfoot>
          ...
          <template v-else-if="gyh === 1 && gyhTableData[1] && gyhTableData[1].table.length">
            <div key="gyh1"
              ...
              <table class="canvas-table"
                ...
                <tbody>
                  <tr v-for="(tr,index) in hezhiTable"
                      :class="index%2 === 1?'bg-grey':''">
                    <td>{{tr.issue}}</td>
                    <td>{{tr.guanyahe}}</td>
                    <td v-for="n in 17">
                      <div :class="tr['sum_'+(n+2)] === 0 ? 'openball' : ''">
                        {{ (tr['sum_'+(n+2)] === 0 ? n+2 : tr['sum_'+(n+2)]) | filter_toTwo }}
                      </div>
                    </td>
                  </tr>
                </tbody>
              ...
              <table style="position: sticky;bottom: 0;z-index: 1;">
                ...
                <tfoot class="bg-yellow">
                  <tr>
                    <td>出现次数</td>
                    <td v-for="value in  gyhTableData[1].analyse.times">
                      {{ value }}
                    </td>
                  </tr>
                  <tr>
                    <td>平均遗漏</td>
                    <td v-for="value in  gyhTableData[1].analyse.pinjunyilou">
                      {{ value }}
                    </td>
                  </tr>
                  <tr>
                    <td>最大遗漏</td>
                    <td v-for="value in  gyhTableData[1].analyse.zuidayilou">
                      {{ value }}
                    </td>
                  </tr>
                  <tr>
                    <td>最大连出</td>
                    <td v-for="value in  gyhTableData[1].analyse.zuidalianchu">
                      {{ value }}
                    </td>
                  </tr>
                </tfoot>
        ...
        <div class="ball-wrap flex">
          <label v-for="(value,key,index) in {'冠亚和两面':0,'和值号码分布':1}"
                 ...
                 class="ball-item ball-item-full">
            <input type="radio"
                   :value="value"
                   v-model="gyh"
                   class="dent-hide-input">
...
export default {
  ...
  computed: {
    ...
    hezhiTable() {
      return (
        this.gyh === 1 && this.gyhTableData[1] && this.gyhTableData[1].table
      );
    }
  },
  methods: {
