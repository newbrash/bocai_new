      ...
      if (this.trading.length == 6) {
        ...
        if (this.trading == this.GLOBAL.userInfo.trading) {
          ...
          this.senddata({
              ...
              if (respone.status) {
                ...
                if(respone.systemNotice){
                  if(that.GLOBAL.msgList.systemNotice){
                    that.$set(that.GLOBAL.msgList.systemNotice, Object.keys(that.GLOBAL.msgList.systemNotice).length, respone.systemNotice);
                  }else{
                    that.$set(that.GLOBAL.msgList, 'systemNotice', {});
                    that.$set(that.GLOBAL.msgList.systemNotice, 0, respone.systemNotice);
                  }
            ...
            callbackFlag: "responseSendRedbagToFriend",
            handType: "user"
          });
        } else {
