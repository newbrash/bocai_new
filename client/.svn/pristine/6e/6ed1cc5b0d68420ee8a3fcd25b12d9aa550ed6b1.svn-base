<template>
  <div style="border: 0px solid green;background:#E7E7E7;min-height:100vh;">
    ...
    <!-- 领红包 -->
    <section v-show="this.GLOBAL.chatRoomShowRedbag" class="open-redbag-box-background"></section>
    <div v-show="this.GLOBAL.chatRoomShowRedbag" class="open-redbag-box">
...
export default {
  ...
  methods: {
    ...
    closeRedbagBox: function() {
      this.$forceUpdate();
      this.GLOBAL.chatRoomShowRedbag = false;
      this.red_id = "";
      this.red_index = "";
    },
    /**
      ...
      try {
        ...
        if (result[1]) {
          ...
          if (result[2].indexOf("-o-" + this.GLOBAL.userInfo.id + "-") === -1) {
            ...
            this.GLOBAL.chatRoomShowRedbag = true;
            this.$forceUpdate();
          } else {
            // 当前用户在抢到红包的人员队列中。跳转红包详情
            this.$router.push({
